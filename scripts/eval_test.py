...
URBANREFER_TEST = json.load(open(os.path.join(CONF.PATH.URBANREFER, "urbanRefer_text_test.json")))

def get_dataloader(args, urbanrefer, all_scene_list, split, config):
    dataset = UrbanReferDataset(
        urbanrefer=urbanrefer, 
...
def get_urbanrefer(args):
    if args.detection:
        scene_list = get_scannet_scene_list("test")
        scanrefer = []
        ...
        scanrefer = URBANREFER_TRAIN if args.use_train else URBANREFER_TEST
...
def eval_ref(args):
    ...
    urbanrefer, scene_list = get_urbanrefer(args)

    # dataloader
    _, dataloader = get_dataloader(args, urbanrefer, scene_list, "test", DC)
        ...
        for seed in seeds:
            ...
            for data in tqdm(dataloader):
                ...
                for i in range(ids.shape[0]):
                    idx = ids[i]
                    scene_id = urbanrefer[idx]["scene_id"]
                    building_id = urbanrefer[idx]["building_id"]

                    if scene_id not in predictions:
                        predictions[scene_id] = {}

                    if building_id not in predictions[scene_id]:
                        predictions[scene_id][building_id] = {}


                    predictions[scene_id][building_id]["pred_bbox"] = data["pred_bboxes"][i]
                    predictions[scene_id][building_id]["gt_bbox"] = data["gt_bboxes"][i]
                    predictions[scene_id][building_id]["iou"] = data["ref_iou"][i]
...
def eval_det(args):
    ...
    # init training dataset
    print("preparing data...")
    scanrefer, scene_list = get_urbanrefer(args)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gpu", type=str, help="gpu", default="6")
